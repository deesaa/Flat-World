...
public class TexturesClass {
	...
	public TexturesClass(String Format, String TexturePath) {
		...
		try {
			TexturesArray.put(NumTextures,TextureLoader.getTexture(Format, ResourceLoader.getResourceAsStream(TexturePath)));
			NumTextures++;
		} catch (IOException e) {
	...
	public void setAnimation(int AnimationID) {
		if (AnimationID < AnimationLists.size()
				&& AnimationID != installedAnimation) {
	...
	public void updateAnimation() {
		...
		if (millsecsPassed >= millsecsToUpdate) {
			...
			if (installedTexture > AnimationLists.get(installedAnimation)[1]) { // Если
																				// это
																				// была
																				// последняя,
																				// то
